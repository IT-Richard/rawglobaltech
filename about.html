...
<head>
  ...
  <title>RAW GLOBAL TECH · About Team</title>
  <!-- Bootstrap, Fonts, Icons, AOS -->
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet">
  <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700;900&family=Orbitron:wght@400;600;700;900&family=Playfair+Display:wght@700&display=swap" rel="stylesheet">
  ...
  <style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    @media (max-width:480px) {
      ...
      .team-avatar { width:80px; height:80px; font-size:2.4rem; }
    }
  </style>
</head>
<body>
...
<script>
  ...
  (function generateParticles() {
    const container = document.querySelector('.section-darker');
    if (!container) return;
    // we only want particles in the background, but we'll add subtle ones
  ...
  // floating shapes (small decoration)
  (function addShapes() {
    const hero = document.querySelector('.section-darker .container');
    if (!hero) return;
    const chars = ['✦','◆','▲','●'];
    for (let i=0; i<3; i++) {
